...
export class AliciaStateManager {
  ...
  private constructor() { this.initServiceWorker(); }
  ...
  private schedule(state: AliciaState, ms: number, onExpire: AliciaState, ctx: Context) {
    setTimeout(() => { if (this.currentState === state) this.transition(onExpire, ctx); }, ms);
  }
  ...
  private emit(type: string, payload: object) {
    document.dispatchEvent(new CustomEvent(`alicia:${type}`, { bubbles: true, detail: { ...payload, timestamp: Date.now() } }));
